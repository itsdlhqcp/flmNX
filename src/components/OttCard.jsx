...
// import Icon from "../assets/icons/prime";
// import PrimeIcon from "../assets/icons/prime";
...
const OttCard = ({ post }) => {
  ...
  return (
    <>
      <div className="relative flex w-full p-2 mt-5 space-x-3 bg-black rounded-t-lg ">
        {/* Image Card */}
        <div className="relative h-[235px] w-full overflow-hidden rounded-lg" onClick={handleCardClick}>
          <img src={IF + post.photo} alt="" className="object-cover w-full h-full" />
          {/* Transparent Widget */}
          <div className="absolute bottom-0 left-0 right-0 p-2 text-white bg-black bg-opacity-50 pointer-events-none">
            ...
            <div className="flex items-center justify-center space-x-2">
                  {post.categories?.map((c, i) => (
              <div key={i} className={`px-3 py-0 rounded-lg text-white font-serif ${c === "#buzz" ? "bg-green-700" : (c === "NETFLEX" ? "bg-red-700" : (c === "#official" ? "bg-blue-700" : "bg-gray-600"))}`}>
               {c}
                      </div>
                     ))}
            </div>
            ...
            <div className="items-center mt-2 space-x-4 font-semibold text-centerflex ">
              <div className="flex items-center justify-center space-x-2">
                {post.categories?.map((c, i) => (
                  <div key={i} className={`px-3 py-0 rounded-lg text-white font-serif ${c === "#buzz" ? "bg-green-700" : (c.toLowerCase() === "netflix" ? "bg-red-700" : (c.toLowerCase() === "prime" ? "" : (c === "#official" ? "bg-blue-700" : "bg-gray-600")))}`}>
                    {c === "Netflix" ? <img src={MailIcon} alt="" /> : (c.toLowerCase() === "prime" ? <MailIcon /> : c)}
                  </div>
                ))}
              </div>
            </div>
      ...
      <Link to={user ? `/ott/${post._id}` : "/login"}>
        <div className="px-0.5 text-white bg-black rounded-b-lg mb-1 pb-2">
          <p className="font-bold text-center ">READ REVIEWS</p>
        </div>
      </Link>
    </>
  );
